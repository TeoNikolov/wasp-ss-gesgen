...
def constraintBoneTargets(armature = 'Armature', rig = 'None', mode = 'full_body'):
    ...
    for bone in bpy.context.selected_pose_bones:
        ...
        if bone.name == 'Hips' and mode == 'full_body':
...
def parse_args():
    ...
    parser.add_argument('-m', "--visualization_mode", help='The visualization mode to use for rendering.',type=str, choices=['full_body'], default='full_body')
...
def main():
    ...
    if bpy.ops.text.run_script.poll():
        ...
        ##################################
        ARG_BVH_PATHNAME = SCRIPT_DIR / 'data' / 'test.bvh'
        ARG_AUDIO_FILE_NAME = SCRIPT_DIR / 'data' / 'test.wav' # set to None for no audio
        ARG_IMAGE = False
        ARG_VIDEO = True
        ARG_START_FRAME = 0
        ARG_DURATION_IN_FRAMES = 9999
        ARG_ROTATE = 'default'
    ...
    # FBX file
    FBX_MODEL = os.path.join(SCRIPT_DIR, 'model', 'LaForgeMale.fbx')
    ...
    constraintBoneTargets(armature = "LaForge_Male_model_v020", rig = BVH_NAME, mode = ARG_MODE)

    if ARG_MODE == "full_body":     CAM_POS = [0, -4.5, 0.1]
